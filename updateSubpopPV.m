...
function subpop = updateSubpopPV(subpop, mem_matrix, global_best, params, g, iter, FES_max, N_UAV, behavior_type)
% 输入参数：
%   behavior_type: 开发行为类型（'turn'=突然转向, 'levy'=随机游走）
    ...
    if nargin < 9 || isempty(behavior_type)
        behavior_type = 'turn';  % 默认值
    end
