...
bar(1:5, hv_means, 0.6, 'FaceColor', [0.0000, 0.4470, 0.7410], 'EdgeColor', 'k', 'LineWidth', 1.2);
hold on;
errorbar(1:5, hv_means, hv_stds, 'k', 'LineStyle', 'none', 'LineWidth', 1.5, 'Capsize', 6);
...
xticklabels(labels);
ylabel('Hypervolume (HV)', 'FontWeight', 'bold');
title('HV Distribution (Robustness Analysis)', 'FontWeight', 'bold');
set(gca, 'FontName', 'Times New Roman', 'FontSize', 12, 'LineWidth', 1.2);
grid on;
...
table_data = cell(n_variants, 5);
for v = 1:n_variants
    variant = variants{v};
    if isfield(results, variant)
        hv_mean = results.(variant).mean_hv;
        hv_std = results.(variant).std_hv;
        table_data{v, 1} = variant_labels{v};
        table_data{v, 2} = sprintf('%.4f±%.4f', hv_mean, hv_std);
        table_data{v, 3} = sprintf('%.4f', p_values(v));
...
uitable('Data', table_data, 'ColumnLabel', col_labels, ...
    'Units', 'normalized', 'Position', [0.05, 0.1, 0.9, 0.85], ...
    'FontName', 'Times New Roman', 'FontSize', 11);

title('Statistical Tests (Wilcoxon Signed-Rank Test)', 'FontName', 'Times New Roman', 'FontWeight', 'bold');
...
fprintf('  - ablation_convergence_hv.fig/png (收敛曲线 + HV柱状图)\n');
fprintf('  - ablation_performance_cost.fig/png (性能vs成本双Y轴图)\n');
fprintf('  - ablation_boxplot.fig/png (HV分布箱型图)\n');
fprintf('  - ablation_statistical_tests.fig/png (统计检验表)\n');
end