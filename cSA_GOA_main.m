...
fprintf('初始化完成：适应度=%.4f, 高优先级覆盖率=%.2f%%, 全局覆盖率=%.2f%%\n', ...
    initial_fit, initial_cov_high*100, initial_cov_total*100);
...
for iter = 2:params.FES_max
    ...
    for g = 1:3
        ...
        % 阶段3：开发阶段
        behavior_types = cell(params.K, 1);
        ...
        is_stagnant = false;
        if iter > 10 && iter > 1
            if cg_curve(iter-1) <= best_fit - improvement_threshold
                is_stagnant = true;
            end
        end
        
        % 确保 mem_matrix 大小与 params.K 匹配（再次检查，以防在自适应调整后K变化）
        current_mem_size = size(mem_matrix{g}, 1);
        if current_mem_size < params.K
            additional_needed = params.K - current_mem_size;
            additional_candidates = sampleCandidates(subpops{g}, additional_needed, N_UAV, Ub, Lb, RRH, ...
        ...
        for i = 1:params.K
            ...
            for uav_idx = 1:N_UAV
                ...
                if capturability_g(g) < adjusted_c && is_stagnant && E_ratio > 0.85
                    use_levy = rand < 0.15;
                end

                if use_levy
                    pos = goaLevy(pos, subpop_best_uav, t);
                    behavior_types{i} = 'e-levy';
                elseif capturability_g(g) >= adjusted_c
                    ...
                    if E_ratio > 0.85
                        if true && is_stagnant
                            ...
                            behavior_types{i} = 'targeted_levy';
                        else
                            pos = goaLevy(pos, subpop_best_uav, t);
                        ...
                        pos = goaTurn(pos, subpop_best_uav, capturability_g(g), t);
                        behavior_types{i} = 'turn_forced_by_energy';
                    end
                end
    ...
    improvement_threshold = max(1.0, priorities_sum * 1e-4);  % 最小改进阈值（优先级和的0.01%或至少1）
    ...
    improvement_threshold_subpop = max(1.0, priorities_sum * 1e-4);  % 子种群改进阈值
    ...
    if mod(iter, 5) == 0 || iter <= 10
        fprintf('迭代 %d/%d, 当前适应度: %.2f (历史最优: %.2f), 剩余能量: %.2f J (平均能耗: %.2f J), 覆盖率: 高优先级=%.2f%%, 全局=%.2f%%\n', ...
            iter, params.FES_max, curr_fit_mean, best_fit, curr_avg_remaining, curr_avg_energy, ...
    ...
    % 记录档案大小并检测更新
    new_archive_size = length(pareto_archive);
    archive_size_history(iter) = new_archive_size;
    pareto_updated_this_iter = (new_archive_size > old_archive_size);
...
function new_pos = goaLevy(pos, global_best_uav, t)
    ...
    mu = randn(1,2);
    v = randn(1,2);
    levy = 0.15 * (mu * sigma_levy) ./ (abs(v).^(1/beta_levy) + 1e-10);
